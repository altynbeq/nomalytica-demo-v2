...
async function fetchDataForRange(startDate, endDate){
    ...
    const url = `/api/ut_uralsk1/hs/sales-kkm-receipts/GetSales/${decodedStartDate}/${decodedEndDate}`;
...
export async function getSalesReceiptsFront(dateRanges) {
    if (Array.isArray(dateRanges) && dateRanges[0]) {
        ...
        // const final = {
        //     filterMonthData: filterData(monthData, monthStart, monthEnd),
        ...
        // };
        const resp = {
            monthReceiptsData: sales1CMonthFormer(monthData),
            weekReceiptsData: sales1CDataFormer(weekData),
            dayReceiptsData: sales1CDataFormer(dayData),
        }
